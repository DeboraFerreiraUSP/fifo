...
{
	while (1) {
		if (k_mutex_lock(&my_mutex_buttom, K_MSEC(1)) == 0 &&
		    k_mutex_lock(&my_mutex_blink, K_MSEC(1)) == 0) {
			printk("Pegou os dois mutex");
			k_mutex_unlock(&my_mutex_buttom);
			k_mutex_unlock(&my_mutex_blink);
		}
		k_mutex_unlock(&my_mutex_buttom);
...
K_THREAD_DEFINE(buttom_id, STACKSIZE, buttom, NULL, NULL, NULL, PRIORITY, 0, 0);
